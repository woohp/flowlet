...
- Concurrent stages emit values in completion order by default. `map(ordered=True)` opts into input order and must stay opt-in: it is what `concurrency=1` would produce, including holding a failure until its input position, which trades a concurrent stage's liveness for determinism. `flat_map` and `filter` have no ordered mode yet.
